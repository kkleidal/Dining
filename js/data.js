...
loadDiningData = function(callback) {
	loadCallback = callback;
	hallsLoaded = new Array(halls.length);
	for (var i = 0; i < hallsLoaded.length; i++) hallsLoaded[i] = false;
	...
	loadMcCormick();
	loadNext();
	// Listen for loading to complete
	waitForLoad();
}
waited = 0;
function waitForLoad() {
	waited++;
	allLoaded = true;
	for (var i = 0; i < hallsLoaded.length; i++) {
		if (hallsLoaded[i] == false) {
			allLoaded = false;
			break;
		}
	}
...
// Prints data for given hall into the results div
function printHallData(hallID) { // hallID:  the DiningHall ID to be printed
	...
	if (mealsServed == 0) outHtml += "<p class=\"desc\">Unfortunately, no meals are currently being served here.  Try back later.</p>";
...
function loadNext() {
	...
	// ^ Claim confirmed...  Residents are also out of range of all cell towers...
